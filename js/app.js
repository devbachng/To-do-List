  ...
  document.querySelectorAll('.task-container').forEach(container => {
    new Sortable(container, {
      ...
      onEnd: (evt) => {
        ...
        if (targetColumn && targetColumn.getAttribute('data-column') === 'done') {
          triggerConfetti();
        }
      }
    });
  ...
  columns.forEach(column => {
    ...
    if (addButton) {
      ...
      addButton.addEventListener('click', () => {
        ...
        const task = { title, description: '', startDate: '', endDate: '', priority: '', subtasks: [] };
        const taskCard = createTaskCard(task);
        taskContainer.appendChild(taskCard);
  ...
  document.getElementById('priorityFilter').addEventListener('change', (e) => {
    ...
    document.querySelectorAll('.card-task').forEach(card => {
      if (selectedPriority === 'all') {
        ...
        card.style.display = (cardPriority === selectedPriority) ? 'block' : 'none';
      }
    });
  });
});
...
function createTaskCard(task) {
  ...
  card.innerHTML = `
    ...
    <small class="text-secondary">Tạo: ${task.startDate || '-'} - ${task.endDate || '-'}</small>
...
document.addEventListener('click', function (e) {
  if (e.target.closest('.card-task') && !e.target.closest('button')) {
    ...
    const title = card.querySelector('strong')?.textContent || '';
    const description = card.querySelector('.text-muted')?.textContent || '';
    const priority = card.classList.contains('high') ? 'Cao' : card.classList.contains('medium') ? 'Trung bình' : card.classList.contains('low') ? 'Thấp' : '';
    const startDate = card.getAttribute('data-start') || '';
    const endDate = card.getAttribute('data-end') || '';
    const subtasks = JSON.parse(card.getAttribute('data-subtasks') || '[]');

    currentTaskData = { title, description, priority, startDate, endDate, subtasks };
...
document.getElementById('saveTaskChanges').addEventListener('click', () => {
  ...
  editingCard.setAttribute('data-subtasks', JSON.stringify(currentTaskData.subtasks));

  const oldProgress = editingCard.querySelector('.progress');
  if (oldProgress) oldProgress.remove();

  if (currentTaskData.subtasks.length > 0) {
    const completed = currentTaskData.subtasks.filter(sub => sub.completed).length;
